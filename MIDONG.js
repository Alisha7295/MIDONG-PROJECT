...
//const login = async function (...Data) { (await require('dhf-horizon-synthesis')('Fca-Horizon-Remake'))(Data); }
const moment = require("moment-timezone");
const timeStart = Date.now();
const axios = require("axios");
const listPackage = JSON.parse(readFileSync('./package.json')).dependencies;
const listbuiltinModules = require("module").builtinModules;
log("Mirai BOT", "Đang khởi tạo các biến...");
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            else return axios.get('https://raw.githubusercontent.com/HieuSimpRin/gbanmirai/main/data.json').then(_0x2f978e => {
    ...
    return axios.get('https://raw.githubusercontent.com/HieuSimpRin/gbanmirai/main/data.json').then(dataGban => {
        ...
        return axios.get('https://raw.githubusercontent.com/HieuSimpRin/gbanmirai/main/data.json').then(json => {
...
var myVar = setInterval(rest,  1000 * 30 * 60);
...
async function connectUptime(url, name) {
	try {
		const res = (await axios.get(`https://api-vip.tuanvudev2.repl.co/upt?add=${url}`)).data;
