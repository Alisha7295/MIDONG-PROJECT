    ...
    return async function ({ event }) {
        ...
        try {
            if (!allThreadID.includes(threadID) && event.isGroup == !![]) {
                ...
                logger(global.getText('handleCreateDatabase', 'newThread', chalk.hex("#" + random3)(`Nhóm mới: `) + chalk.hex("#" + random4)(`${threadID}`) + "  ||  " + chalk.hex("#" + random5)(`${threadIn4.threadName}`)), '[ THREAD ]');
            }
            if (!allUserID.includes(senderID) || !userName.has(senderID)) {
                const infoUsers = await Users.getInfo(senderID), setting3 = {};
                setting3.name = infoUsers.name;
                await Users.createData(senderID, setting3);
                allUserID.push(senderID);
                userName.set(senderID, infoUsers.name);
                logger(global.getText('handleCreateDatabase', 'newUser', chalk.hex("#" + random6)(`Người dùng mới: `) + chalk.hex("#" + random7)(`${singleData.name}`) + " || " + chalk.hex("#" + random8)(`${senderID}`)), '[ USER ]');
            }
