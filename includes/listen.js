 ...
 const checkttDataPath = __dirname + '/../modules/commands/checktuongtac/';
  setInterval(async() => {
    ...
    if (day != day_now) {
      ...
      api.sendMessage({body:checkttBody,attachment: fsfs.createReadStream(__dirname + `/../modules/commands/noprefix/ttngay.gif`)},  checkttFile.replace('.json', ''), (err) => err ? console.log(err) : '');
...
url: (await global.nodemodule["axios"]('https://API-RYO.ducryovn.repl.co/api/anime.php')).data.data,
method: "GET",
responseType: "stream"
})).data
