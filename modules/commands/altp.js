...
var path = __dirname + "/cache/1.jpg";
    let getimg = (await axios.get(`https://i.imgur.com/bRn1OEb.jpg`, { responseType: 'arraybuffer' })).data;
  fs.writeFileSync(path, Buffer.from(getimg, "utf-8"));
  
  var msg = "〉===『 𝗔𝗜 𝗟𝗔̀ 𝗧𝗥𝗜𝗘̣̂𝗨 𝗣𝗛𝗨́ 』=== 〈" + "\n\n"
+ prefix + "𝗮𝗹𝘁𝗽 𝗿𝗲𝗴𝗶𝘀𝘁𝗲𝗿 ➩ Đăng kí chương trình (cần 1000$)" + "\n"
+ prefix + "𝗮𝗹𝘁𝗽 𝗽𝗹𝗮𝘆 ➩ " + playto + "\n"
+ prefix + "𝗮𝗹𝘁𝗽 𝗶𝗻𝗳𝗼 ➩ Xem thông tin câu hỏi và tiền thưởng" + "\n"
+ prefix + "𝗮𝗹𝘁𝗽 𝘁𝗼𝗽 [𝗯𝗼𝘅/𝘀𝗲𝘃𝗲𝗿] ➩ Xem hạng level box và sever" + "\n"
+ prefix + "𝗮𝗹𝘁𝗽 𝘀𝘁𝗼𝗽 ➩ Dừng chơi và nhận tiền thưởng"
  if (ADMINBOT.includes(senderID)) msg += `\n${prefix}𝗮𝗹𝘁𝗽 𝘀𝗲𝘁𝗹𝘃 @ ➩ Set level của @tag (dành riêng admin)\n\n`;
  msg += "      ======[ " + gio + " ]=====";
  if (args.length == 0) return api.sendMessage({ body: msg , attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, event.messageID);
