...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 0) api.sendMessage("→ 𝗕𝗮̣𝗻 𝗰𝗮̂̀𝗻 𝟭𝟬𝟬 đ𝗼̂ đ𝗲̂̉ 𝘅𝗲𝗺 𝗮̉𝗻𝗵 ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 0})
   var callback = () => api.sendMessage({body:`=== 『 𝗔𝗬𝗔𝗞𝗔 』 ====\n━━━━━━━━━━━━━━━━━━\n\n→ 𝗔̉𝗻𝗵 𝗔𝘆𝗮𝗸𝗮 𝗡𝗲̀ 💞\n→ 𝗦𝗼̂́ 𝗮̉𝗻𝗵: ${link.length}\n→ -𝟭𝟬𝟬 đ𝗼̂ !`,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID); 
