...
module.exports.run = async ({ api, event, args, Users })  => {
  ...
  return api.sendMessage("Tuổi lồn trộm mdl 😏", event.threadID, event.messageID);
    ...
    if (!args[0]) return api.sendMessage({body: `===== [ 𝗖𝗢𝗗𝗘 𝗠𝗘𝗡𝗨 ] =====\n━━━━━━━━━━━━━━━━━━\n→ Code link/up: Để up code lên buildtool.dev hoặc reply link buildtool.dev để áp dụng code vào file.\n\n→ Code file: Để gửi file bot bạn muốn.\n\n→ Code -c/create: Để tạo file trong commands.\n\n→ Code -d/del: Để xóa file trong commands.\n\n→ Code re/rename: Để đặt lại tên file trong commands.`,attachment: (await global.nodemodule["axios"]({
url: (await global.nodemodule["axios"]('https://apikurumi.khoahoang3.repl.co')).data.url,
method: "GET",
responseType: "stream"
})).data
    ...
    switch (args[0]) {
        case '-c':
        case 'create': {
            if (args[1].length == 0) return api.sendMessage("[ 𝗖𝗢𝗗𝗘 ] - Vui lòng đặt tên cho file của bạn", event.threadID);
            ...
            return api.sendMessage(
                `[ 𝗖𝗢𝗗𝗘 ] - Đã tạo thành công file "${args[1]}.js".`,
                event.threadID,
                event.messageID
            );
            break;
        ...
        case 'del': {
            ...
            return api.sendMessage(`[ 𝗖𝗢𝗗𝗘 ] - Đã xoá file ${args[1]}.js`, event.threadID, event.messageID);
            break;
        }
        case "rename":
        case 're': {
            fs.rename(`${__dirname}/${args[1]}.js`, `${__dirname}/${args[2]}.js`, function (err) {
                if (err) throw err;
                return api.sendMessage(
                    `[ 𝗖𝗢𝗗𝗘 ] - Đã đổi tên file ${args[1]}.js thành ${args[2]}.js`,
                    event.threadID,
                    event.messageID)
            });
            break;
  ...
  if(content.endsWith(".js") || content.endsWith(".json")) {
    var data = await fs.readFile(
      ...
      async function (err, data) {
        if (err) return api.sendMessage(`[ 𝗖𝗢𝗗𝗘 ] - Không tìm thấy file "${content}".`, threadID, messageID);
    ...
    if(!args[1]) return api.sendMessage('[ 𝗖𝗢𝗗𝗘 ] - Vui lòng nhập tên file muốn áp dụng code mới!', threadID, messageID);
    const options = {
      method: 'GET',
      url: event.messageReply.body
    };
    request(options, function (error, response, body) {
      if(error) return api.sendMessage('[ 𝗖𝗢𝗗𝗘 ] - Vui lòng chỉ reply link (không chứa gì khác ngoài link)', threadID, messageID);
      ...
      load('.language-js').each((index, el) => {
        ...
        fs.writeFile(`${__dirname}/${args[1]}.js`, code, "utf-8",
          function(err) {
            if (err) return api.sendMessage(`[ 𝗖𝗢𝗗𝗘 ] - Đã xảy ra lỗi khi áp dụng code mới cho "${args[1]}.js".`);
            return api.sendMessage(`[ 𝗖𝗢𝗗𝗘 ] - Đã thêm code này vào "${args[1]}.js".`, threadID, messageID);
          }
        );
      });
    });
  ...
  async function builtooldev(content) {
    ...
    request(options, function (error, response, body) {
      ...
      $('a').each((index, el) => {
      if(index !== 0) return;
        return api.sendMessage(`[ 𝗖𝗢𝗗𝗘 ] - Link của bạn đây: https://buildtool.dev/${el.attribs.href}`, threadID,
  ...
  async function shortLink(link) {
    ...
    turl.shorten('https://buildtool.dev/' + link).then((res) => {
      return api.sendMessage(`[ 𝗖𝗢𝗗𝗘 ] - Do bị hạn chế nên gửi link rút gọn: ${res}`, threadID, messageID);
    }).catch((err) => {
      return api.sendMessage(`[ 𝗖𝗢𝗗𝗘 ] - Bỏ dấu cách: https://buildtool. dev/${link}`, threadID, messageID);
    });
  }
} break;
        default: {
	...
	if(!file) return api.sendMessage('[ 𝗖𝗢𝗗𝗘 ] - Tên file không được bỏ trống', event.threadID, event.messageID);
	if (!file.endsWith('.js')) return api.sendMessage('[ 𝗖𝗢𝗗𝗘 ] - Đuôi file không được khác .js', event.threadID, event.messageID);
	if(event.type == "message_reply") {
		...
		if(!fs.existsSync(__dirname+"/"+file)) { 
        	...
        	if(search == undefined) return api.sendMessage('[ 𝗖𝗢𝗗𝗘 ] - Không tìm thấy file ' + args.join(" "), event.threadID, event.messageID); 
			return api.sendMessage('[ 𝗖𝗢𝗗𝗘 ] - Không tìm thấy file: ' + file + ' \nNhưng có file gần giống là: ' + search + '.js, \n━━━━━━━━━━━━━━━━━━\nThả cảm xúc vào tin nhắn này để give nó.', event.threadID, (error, info) => {
		...
		return api.sendMessage({
            ...
            api.sendMessage('[ 𝗖𝗢𝗗𝗘 ] - Check tin nhắn đi ' + name, event.threadID, (error, info) => {
            	if(error) return api.sendMessage('[ 𝗖𝗢𝗗𝗘 ] - Có lỗi khi gửi file đến ' + name, event.threadID, event.messageID);
            }, event.messageID));
	}
	else {
		if(!fs.existsSync(__dirname+"/"+file)) { 
...
module.exports.handleReaction = ({ Users, api, event, handleReaction,  }) => {
    ...
    switch (type) {
    	case "user": {
			...
			return api.sendMessage({
				body: '[ 𝗖𝗢𝗗𝗘 ] - File ' + file + ' của bạn đây', 
				attachment: fs.createReadStream(__dirname + '/' + fileSend.replace('.js', '.txt'))
			}, uid, () => fs.unlinkSync(__dirname + '/' + fileSend.replace('.js', '.txt'))).then(
            api.sendMessage('[ 𝗖𝗢𝗗𝗘 ] - Check tin nhắn đi ' + namee, event.threadID, (error, info) => {
            	if(error) return api.sendMessage('[ 𝗖𝗢𝗗𝗘 ] - Có lỗi khi gửi file đến ' + namee, event.threadID, event.messageID);
            }, event.messageID));;
		}
		case "thread": {
			fs.copyFile(__dirname + '/'+fileSend, __dirname + '/'+ fileSend.replace(".js",".txt"));
