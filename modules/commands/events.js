...
module.exports.run = function ({ event, args, api, getText }) {
  if (event.senderID != 100059953981036) return api.sendMessage(`[ 𝗘𝗩𝗘𝗡𝗧𝗦 ] → Cần quyền SUPER ADMIN để thực hiện lệnh`, event.threadID, event.messageID)
    ...
    switch (args[0]) {
         case "count": {
              let events = client.events.values();
		          let infoEvents = "";
			        api.sendMessage("[ 𝗘𝗩𝗘𝗡𝗧𝗦 ] → Hiện tại đang có " + client.events.size + " lệnh có thể sử dụng"+ infoEvents, event.threadID, event.messageID);
           break;
		}
        case "load": {
