...
var url = res.data.data.music;
    ...
    case "i":{
      ...
      if (!username) return api.sendMessage("con mẹ m info đâu", threadID);
       const res = await axios.get(`https://api-caochungdat.bokdepzai.repl.co/tiktok/infouser?uid=${username}`);
      var url1 = res.data.userInfo.user.avatarLarger;
  var callback = () => api.sendMessage({body:`Name:${res.data.userInfo.user.nickname}\nTiểu sử: ${res.data.userInfo.user.signature}\nFL:${res.data.userInfo.stats.followerCount}\nĐang fl:${res.data.userInfo.stats.followingCount}\nThích:${res.data.userInfo.stats.heart}\nSố video:${res.data.userInfo.stats.videoCount}`,attachment: fs.createReadStream(__dirname + "/cache/tkinfo.png")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/tkinfo.png"),event.messageID);
